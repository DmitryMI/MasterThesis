...
\begin{figure}
  	\caption{3D view on the simulation.}
	\centering
...
\cref{tab:simulationparams} shows values of simulation and mobility parameters.  Note that Vehicle Speed is actually defined by the road grid speed limits and traffic conditions. The value shown here is the intended speed on a regular road.
...
\begin{table}
    ...
    \begin{tabular}{rlcl}
		...
		Vehicle Weighted p-persistence \ac{ATR} & - & \SI{150}{\meter} & - \\
		...
		Drone Weighted p-persistence \ac{ATR} & - & \SI{500}{\meter} & - \\
        ...
        \bottomrule
    \end{tabular}
    \caption{Static simulation parameters. Const column means that this value is constant (non-random). Min-Max columns show that this value varies between Minimum and Maximum.}
