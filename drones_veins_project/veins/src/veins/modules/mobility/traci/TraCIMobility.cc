...
const simsignal_t TraCIMobility::parkingStateChangedSignal = registerSignal(
		"org_car2x_veins_modules_mobility_parkingStateChanged");
...
void TraCIMobility::initialize(int stage)
{
	if (stage == 0)
	{
		...
		if (accidentCount > 0)
		{
			double accidentProbability = par("accidentProbability").doubleValue();
			double dice = uniform(0.0, 1.0, 0);
			if (accidentProbability > dice)
			{
...
void TraCIMobility::nextPosition(const Coord &position, std::string road_id, double speed, Heading heading,
		VehicleSignalSet signals)
{
	EV_DEBUG << "nextPosition " << position.x << " " << position.y << " " << road_id << " " << speed << " " << heading
			<< std::endl;
	isPreInitialized = false;
	this->roadPosition = position;
	this->road_id = road_id;
